...
class SystemMonitor:
    """
    Handles retrieval of system metrics using standard library tools.
    Designed for Linux systems (reading /proc) with fallbacks.
    """
    def __init__(self):
        self.os_type = platform.system()
        self.start_time = time.time()
        self.cpu_prev: Dict[str, int] = {}
        self.net_prev: Dict[str, float] = {}
    ...
    def get_memory(self) -> Dict[str, Any]:
        """Parses /proc/meminfo for Linux memory stats."""
        try:
                ...
                with open('/proc/meminfo', 'r') as f:
                    ...
                    for line in f:
                        ...
                        if key in ['MemTotal', 'MemAvailable']:
                            mem_data[key] = int(parts[1]) * 1024  # Convert kB to Bytes
                    
                    total = mem_data.get('MemTotal', 0)
                    avail = mem_data.get('MemAvailable', 0)
                    used = total - avail
    ...
    def get_cpu_usage(self) -> float:
        """Calculates CPU usage based on /proc/stat deltas."""
        try:
            if self.os_type == "Linux":
                with open('/proc/stat', 'r') as f:
                    ...
                    usage = 0
                    if self.cpu_prev:
                        delta_idle = idle - self.cpu_prev['idle']
                        delta_total = total - self.cpu_prev['total']
                        if delta_total > 0:
                            usage = 100 * (1 - delta_idle / delta_total)
                    
                    self.cpu_prev = {'idle': idle, 'total': total}
    ...
    def get_network_io(self) -> Dict[str, float]:
        """Parses /proc/net/dev for network traffic."""
        try:
            if self.os_type == "Linux":
                with open('/proc/net/dev', 'r') as f:
                    ...
                    for line in f.readlines()[2:]:
                        if ':' in line and 'lo' not in line: # Exclude loopback
                    ...
                    if self.net_prev:
                        time_delta = current_time - self.net_prev['time']
                        if time_delta > 0:
                            rx_rate = (rx_bytes - self.net_prev['rx']) / time_delta
                            tx_rate = (tx_bytes - self.net_prev['tx']) / time_delta
                    
                    self.net_prev = {'rx': rx_bytes, 'tx': tx_bytes, 'time': current_time}
                    return {'rb': rx_bytes, 'tb': tx_bytes, 'rr': rx_rate, 'tr': tx_rate}
        except Exception:
            pass
        return {'rb': 0, 'tb': 0, 'rr': 0, 'tr': 0}
    ...
    def generate_events(self, cpu: float, mem: float, proc: int, net_rx: float) -> List[Dict[str, str]]:
        """Generates system logs based on thresholds."""
        events = []
        
        # CPU Events
        if cpu != self.last_cpu_pct:
            if cpu > 80:
                events.append({'msg': f'High CPU usage: {cpu:.1f}%', 'typ': 'warn'})
            elif cpu > self.last_cpu_pct + 15:
                events.append({'msg': f'CPU spike detected: {cpu:.1f}%', 'typ': 'warn'})
            elif cpu < self.last_cpu_pct - 15:
                events.append({'msg': f'CPU load decreased: {cpu:.1f}%', 'typ': 'info'})
            self.last_cpu_pct = cpu

        # Memory Events
        if mem != self.last_mem_pct:
            if mem > 85:
                events.append({'msg': f'High memory usage: {mem:.1f}%', 'typ': 'warn'})
            elif mem > self.last_mem_pct + 10:
                events.append({'msg': f'Memory increased: {mem:.1f}%', 'typ': 'info'})
            self.last_mem_pct = mem

        # Process Events
        if proc != self.last_proc_count:
            if proc > self.last_proc_count:
                events.append({'msg': f'Process spawned (total: {proc})', 'typ': 'info'})
            elif proc < self.last_proc_count:
                events.append({'msg': f'Process terminated (total: {proc})', 'typ': 'info'})
            self.last_proc_count = proc

        # Network Events
        if net_rx > self.last_net_rx + 50000: # 50KB spike
            events.append({'msg': f'Network activity: {net_rx/1024:.0f} KB/s', 'typ': 'info'})
            self.last_net_rx = net_rx

        # Keep-alive heartbeat
        if not events and int(time.time()) % 5 == 0:
            events.append({'msg': 'System monitoring active', 'typ': 'info'})
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'mem': {
                ...
                'use': mem['used'],
                'avl': mem['available'],
                'pct': round(mem['percent'], 2)
            },
            'disk': {
                'tot': disk['total'],
                'use': disk['used'],
                'fre': disk['free'],
                'pct': round(disk['percent'], 2)
            },
            'net': {
                'rb': net['rb'],
                'tb': net['tb'],
            ...
            'sys': {
                ...
                'proc': (platform.processor()[:30] if platform.processor() else 'Unknown')
            },
            'ev': self.generate_events(cpu_pct, mem['percent'], proc_count, net['rr'])
        }
...
class RequestHandler(http.server.BaseHTTPRequestHandler):
    """
    Serves the Cyberpunk Dashboard and the JSON Metrics API.
    """
    def do_GET(self):
        if self.path in ('/', ''):
            ...
            self.send_header('Content-Security-Policy', "default-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://fonts.gstatic.com https://cdn.jsdelivr.net")
            self.send_header('Cache-Control', 'no-store')
            self.end_headers()
            self.wfile.write(WEB_INTERFACE.encode('utf-8'))
            ...
            self.send_header('Cache-Control', 'no-store')
            self.end_headers()
            data = json.dumps(monitor.get_all_metrics())
    ...
    def log_message(self, format, *args):
        # Suppress console logging for every request to keep terminal clean
        pass

class ThreadedHTTPServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    """Multi-threaded server to handle concurrent requests without lag."""
...
if __name__ == '__main__':
    ...
    with ThreadedHTTPServer((HOST, PORT), RequestHandler) as httpd:
        print(f'[SYSTEM_MONITOR] Running at http://{HOST}:{PORT}')
        print('[CTRL+C] to shutdown')
        try:
            httpd.serve_forever()
        except KeyboardInterrupt:
            print('\n[SHUTDOWN] System Monitor stopped.')
